...
num=201
input_video = r'D:/project/spilt_pic/video/3/'+str(num)+'.mp4'
output_dir = r'D:/project/spilt_pic/video/3/img/'


START_TIME = 0  # 设置开始时间(单位秒)
END_TIME = 1797  # 设置结束时间(单位秒)
...
while success and frametoStop >= count:
    if count % (fps * seconds) == 0:  # 每second秒保存一次
        save_path = output_dir +str(num)+'_'+ str(count) + ".jpg"
